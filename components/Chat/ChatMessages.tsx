            ...
            <Stack gap={4} style={{ flex: 1 }}>
              ...
              <Box>
                {m.parts.map((part, i) => {
                  if (part.type === "text") {
                    return (
                      <Text key={i} size="md" c="rgba(255,255,255,0.9)" style={{ lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
                        {part.text}
                      </Text>
                    );
                  }
                ...
                <Box key={toolCallId ?? i} ml="3rem" mt="sm">
                  <Paper withBorder radius="lg" p="md" style={{ background: "rgba(0,0,0,0.2)", borderColor: "rgba(147,51,234,0.2)" }}>
                      ...
                      <Table verticalSpacing="xs" striped highlightOnHover>
                        <Table.Thead>
                          <Table.Tr style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                            {Object.keys(result.data[0]).map((key: string) => (
                              <Table.Th key={key} style={{ fontSize: "11px", color: "rgba(255,255,234,0.4)", textTransform: "uppercase" }}>{key}</Table.Th>
                            ))}
                          </Table.Tr>
                        </Table.Thead>
                        <Table.Tbody>
                          {result.data.slice(0, 10).map((row: any, ri: number) => (
                            <Table.Tr key={ri}>
                              {Object.values(row).map((val: any, ci: number) => (
                                <Table.Td key={ci} style={{ fontSize: "12px", color: "white" }}>{String(val)}</Table.Td>
                              ))}
                            </Table.Tr>
                          ))}
                        </Table.Tbody>
                    ...
                    {result.data.length > 10 && (
                      <Text size="11px" ta="center" mt="xs" c="dimmed">Showing first 10 rows of {result.data.length}</Text>
                    )}
                  </Paper>
                </Box>
              );
