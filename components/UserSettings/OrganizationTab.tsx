...
const largeInputStyles = {
  ...
  input: {
    backgroundColor: "var(--orcha-surface)",
...
export function OrganizationTab() {
  ...
  if (!organization) {
    return (
      <Paper withBorder p="3rem" radius="xl" style={{ background: "var(--orcha-panel)", borderColor: "var(--orcha-border)" }}>
        <Center style={{ height: "250px" }}>
          <Stack align="center" gap="md">
            <IconBuildingSkyscraper size={56} color="var(--orcha-purple)" />
            <Text c="var(--orcha-text-title)" fw={700} size="lg">No active organization selected</Text>
            <Text size="sm" c="var(--orcha-text-muted)" ta="center" maw={450}>Please select or create a Clerk organization to manage its settings.</Text>
          </Stack>
        </Center>
      </Paper>
    );
  ...
  return (
    <Stack gap="2.5rem">
      ...
      <Paper withBorder p="3rem" radius="xl" style={{ background: "var(--orcha-panel)", borderColor: "var(--orcha-border)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.05)" }}>
        <Stack gap="2rem">
          <Group gap="md">
            <IconBuildingSkyscraper size={28} color="#a855f7" />
            <Title order={3} size="1.8rem" c="var(--orcha-text-title)" style={{ letterSpacing: "-0.02em" }}>{organization.name}</Title>
          </Group>
        ...
        <Stack gap="xl">
          <Group justify="space-between">
            <Group gap="sm">
              ...
              <Title order={4} size="1.4rem" c="var(--orcha-text-title)" style={{ letterSpacing: "-0.01em" }}>Members</Title>
              {memberships?.data && (
          ...
          {memberships?.isLoading ? (
            ...
            <Table variant="unstyled" style={{ color: "var(--orcha-text-body)" }}>
              <Table.Thead style={{ borderBottom: "1px solid var(--orcha-border)", height: "45px" }}>
                <Table.Tr>
                  <Table.Th style={{ color: "var(--orcha-text-title)", fontWeight: 600, fontSize: "14px" }}>User</Table.Th>
                  <Table.Th style={{ color: "var(--orcha-text-title)", fontWeight: 600, fontSize: "14px" }}>Email</Table.Th>
                  <Table.Th style={{ color: "var(--orcha-text-title)", fontWeight: 600, fontSize: "14px" }}>Role</Table.Th>
                  {isAdmin && <Table.Th style={{ textAlign: "right", color: "var(--orcha-text-title)", fontWeight: 600, fontSize: "14px" }}>Actions</Table.Th>}
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
                {memberships?.data?.map((m) => {
                  ...
                  return (
                    <Table.Tr key={m.id} style={{ borderBottom: "1px solid var(--orcha-table-border)" }}>
                      <Table.Td py="md">
                        <Group gap="sm">
                          <Avatar src={m.publicUserData.imageUrl} radius="xl" size="md" />
                          <Text size="md" fw={600} c="var(--orcha-text-title)">
                      ...
                      <Table.Td py="md">
                        <Text size="md" c="var(--orcha-text-body)">{m.publicUserData.identifier}</Text>
                      </Table.Td>
                      <Table.Td py="md">
                        {isAdmin && !isSelf ? (
                          <Select
                            ...
                            size="sm"
                            styles={{
                              input: { color: "var(--orcha-purple)", fontWeight: 700, fontSize: "14px", height: "36px" },
                              dropdown: { background: "var(--orcha-panel)", borderColor: "var(--orcha-border)", borderRadius: "8px" },
                              option: { color: "var(--orcha-text-title)", fontSize: "13px", padding: "8px 12px" }
                            }}
                          />
                        ) : (
        ...
        <Paper withBorder p="3rem" radius="xl" style={{ background: "var(--orcha-panel)", borderColor: "var(--orcha-border)", boxShadow: "0 8px 32px rgba(0, 0, 0, 0.05)" }}>
          <Stack gap="xl">
            <Group gap="sm">
              ...
              <Title order={4} size="1.4rem" c="var(--orcha-text-title)" style={{ letterSpacing: "-0.01em" }}>Pending Invitations</Title>
            ...
            <Table variant="unstyled" style={{ color: "var(--orcha-text-body)" }}>
              <Table.Thead style={{ borderBottom: "1px solid var(--orcha-border)", height: "45px" }}>
                <Table.Tr>
                  <Table.Th style={{ color: "var(--orcha-text-title)", fontWeight: 600, fontSize: "14px" }}>Invited Email</Table.Th>
                  <Table.Th style={{ color: "var(--orcha-text-title)", fontWeight: 600, fontSize: "14px" }}>Role</Table.Th>
                  {isAdmin && <Table.Th style={{ textAlign: "right", color: "var(--orcha-text-title)", fontWeight: 600, fontSize: "14px" }}>Actions</Table.Th>}
                </Table.Tr>
              </Table.Thead>
              <Table.Tbody>
                {invitations.data.map((inv) => (
                  <Table.Tr key={inv.id} style={{ borderBottom: "1px solid var(--orcha-table-border)" }}>
                    <Table.Td py="md">
                      <Text size="md" fw={600} c="var(--orcha-text-title)">{inv.emailAddress}</Text>
                    </Table.Td>
                    <Table.Td py="md">
      ...
      <Modal
        ...
        title={
          <Group gap="xs">
            ...
            <Text fw={800} size="lg" c="var(--orcha-text-title)" style={{ letterSpacing: "-0.01em" }}>Invite New Member</Text>
          </Group>
        }
        centered
        styles={{
          content: { background: "var(--orcha-panel)", border: "1px solid var(--orcha-border)", borderRadius: "16px", padding: "1.5rem" },
          header: { background: "var(--orcha-panel)", borderBottom: "1px solid var(--orcha-border)", paddingBottom: "15px" },
        }}
      >
        <Stack gap="2rem" pt="md">
          <Text size="md" c="var(--orcha-text-body)" style={{ lineHeight: 1.5 }}>
