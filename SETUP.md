...
> [!CAUTION]
> **All keys, secrets, and credentials shown in this document are for local development only.** They are auto-generated by the local Docker containers and have no access to any production, staging, or cloud environment. They are safe to commit to the repository and will not work outside of `localhost`. **Never reuse local development keys in production.**

### One-Command Setup (Recommended)
Run the automated bootstrap script — it handles everything (Docker, admin keys, Convex deploy, env sync):
```powershell
.\setup.ps1
```
> Use `.\setup.ps1 -SkipBuild` to skip Docker image rebuilds, or `.\setup.ps1 -SkipDeps` to skip `npm install`.
...
# NOTE: The admin key below is a LOCAL-ONLY key generated by your Docker container.
#       It has zero access to any cloud or production system.
npx.cmd convex dev --url http://localhost:3210 --admin-key "convex-self-hosted|YOUR_ADMIN_KEY"
```
...
> [!NOTE]
> The `ENCRYPTION_KEY` and `CLERK_ISSUER_DOMAIN` values below are **local development defaults**. They encrypt data only within your local Docker volume and authenticate against a development-only Clerk instance. They grant no access to any production system.

In a separate terminal, run:
```bash
# 1. Set the database credentials encryption key (local dev only)
npx convex env set ENCRYPTION_KEY "YOUR_ENCRYPTION_KEY" --url http://localhost:3210 --admin-key "convex-self-hosted|YOUR_ADMIN_KEY"

# 2. Set the Clerk issuer domain (local dev only — matches CLERK_ISSUER_DOMAIN in .env)
npx convex env set CLERK_ISSUER_DOMAIN "YOUR_CLERK_ISSUER_DOMAIN" --url http://localhost:3210 --admin-key "convex-self-hosted|YOUR_ADMIN_KEY"
```
...
> ```powershell
> node node_modules\convex\bin\main.js env set ENCRYPTION_KEY "YOUR_ENCRYPTION_KEY" --url "http://localhost:3210" --admin-key "convex-self-hosted|YOUR_ADMIN_KEY"
> node node_modules\convex\bin\main.js env set CLERK_ISSUER_DOMAIN "YOUR_CLERK_ISSUER_DOMAIN" --url "http://localhost:3210" --admin-key "convex-self-hosted|YOUR_ADMIN_KEY"
> ```


### 5c. Deploy Convex Schema & Functions (CRITICAL for Fresh Instances)
A fresh self-hosted Convex backend starts with an empty database — no tables, indexes, or query/mutation functions exist yet. You **must** push the local schema and function code before the app can work:
```bash
npx convex deploy --url http://localhost:3210 --admin-key "convex-self-hosted|YOUR_ADMIN_KEY"
```
> 🛠️ **Windows Troubleshooting (Pipe Escaping Error):**
> Use the same Node-based workaround as Step 5b:
> ```powershell
> node node_modules\convex\bin\main.js deploy --url "http://localhost:3210" --admin-key "convex-self-hosted|YOUR_ADMIN_KEY"
> ```

*Wait for all indexes to finish backfilling (this can take 1-2 minutes). You will see `✔ Deployed Convex functions` when complete.*
